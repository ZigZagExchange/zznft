...
const Modal = ({children, open, title, description, onChange}: DialogProps) => {
  return <RadixDialog.Root open={open} onOpenChange={(value) => onChange(value)}>
    <RadixDialog.Portal>
      ...
      <RadixDialog.Content
        style={{transform: "translate(-50%, -50%)", maxWidth: '500px'}}
        className={css("bg-neutral-900", "rounded-sm", "top-1/2", "left-1/2", "fixed", "w-9/12", "p-10")}>
        <RadixDialog.Close style={{right: "5px", top: "5px"}}
                           className={css("absolute", "text-neutral-400", "hover:text-white")}>
          <MdClose size={"25px"}/>
        </RadixDialog.Close>
        <RadixDialog.Title className={css("text-white", "font-mono", "text-2xl", "font-bold", "text-center", "mb-6")}>
          {title}
        </RadixDialog.Title>
